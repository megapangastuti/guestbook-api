...
const usersModel = require('../../models/guestbook/users');
...
try {
  ...
  if (hasComponents) {
    ...
    const componentConfig = {
      tableName: 'users',
      fieldName: ["id","name","email","password","role","created_at","updated_at"],
      exportQuery: null,
      columnFormats: null,
      fieldLabels: null,
    ...
    componentEngine.loadConfigurationFromObject(componentConfig).then(result => {
      if (result.success) {
        console.log(`Component configuration loaded for guestbook/users: ${result.componentsLoaded} components`);
      }
    }).catch(err => {
      console.error(`Failed to load component configuration for guestbook/users:`, err.message);
    });
  } else {
    console.log(`No components defined in payload for guestbook/users, running without events`);
  }
} catch (e) {
  if (hasComponents) {
    // Components dikonfigurasi tapi engine gagal load — ini error, bukan optional
    console.error(`CRITICAL: Component engine failed to load for guestbook/users but components are configured:`, e.message);
    throw e;
  }
  // Jika tidak ada components, silent skip wajar
  console.log(`No component engine required for guestbook/users, running without events`);
}
    ...
    try {
      ...
      if (!req.body || Object.keys(req.body).length === 0) {
        return res.status(400).json({
          ...
          endpoint: '/api/guestbook/users' + req.path
        });
      }
...
// POST /api/guestbook/users/datatables - Data untuk DataTables
router.post('/datatables', async (req, res) => {
  try {
    // req.log.debug('Request body users/datatables:', JSON.stringify(req.body, null, 2));
    ...
    const result = await usersModel.getDatatables(options);
    ...
    console.error('Error in users datatables:', error);
    const statusCode = error.statusCode || 500;
    return res.status(statusCode).json({
      success: false,
      error: statusCode === 400 ? 'Bad Request' : 'Internal Server Error',
      message: statusCode === 400 ? error.message : 'An error occurred while fetching users data',
...
// POST /api/guestbook/users/create - Menambahkan data users baru
router.post('/create', async (req, res) => {
  try {
    // req.log.debug('Request body users/create:', JSON.stringify(req.body, null, 2));
    ...
    if (typeof usersModel.validateData === 'function') {
      const validation = await usersModel.validateData(req.body, 'insert');
    ...
    if (componentEngine && ContextBuilder) {
      ...
      try {
        ...
        responseData = await usersModel.addData(req.body, eventContext);
      ...
      try {
        responseData = await usersModel.addData(req.body, { additionalContext: { requestId: req.id || null } });
        newData = responseData;
    ...
    console.log(`users data added successfully: ${responseData.id || 'new record'}`);

    // Kirim response
    return res.status(201).json({
      success: true,
      message: 'users data successfully added',
      data: responseData,
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error('Error saat menambahkan data users:', error);
    ...
    return res.status(500).json({
      ...
      message: 'An error occurred while adding users data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// POST /api/guestbook/users/update - Mengupdate data users
router.post('/update', async (req, res) => {
  try {
    // req.log.debug('Request body users/update:', JSON.stringify(req.body, null, 2));
    ...
    // Validasi data dengan model jika tersedia
    if (typeof usersModel.validateData === 'function') {
      const validation = await usersModel.validateData(req.body, 'update');
      if (!validation.isValid) {
        return res.status(400).json({
          success: false,
          error: 'Validation failed',
    ...
    if (componentEngine && ContextBuilder) {
      ...
      try {
        ...
        responseData = await usersModel.updateData(req.body, eventContext);
        newData = responseData;
      ...
      // Fallback: gunakan mode lama tanpa events (tetap propagasi requestId untuk Live Sync)
      try {
        responseData = await usersModel.updateData(req.body, { additionalContext: { requestId: req.id || null } });
        newData = responseData;
    ...
    // Log successful operation
    console.log(`users data updated successfully: ${primaryKey}=${req.body[primaryKey]}`);
    ...
    console.error('Error saat mengupdate data users:', error);
    ...
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while updating users data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// POST /api/guestbook/users/delete - Menghapus data users
router.post('/delete', async (req, res) => {
  try {
    // req.log.debug('Request body users/delete:', JSON.stringify(req.body, null, 2));
    ...
    if (req.body.where && Array.isArray(req.body.where) && req.body.where.length > 0) {
      ...
      try {
        const existingData = await usersModel.getData({
    ...
    if (componentEngine && ContextBuilder) {
      ...
      try {
        ...
        responseData = await usersModel.deleteData(req.body, eventContext);
      ...
      // Fallback: gunakan mode lama tanpa events (tetap propagasi requestId untuk Live Sync)
      try {
        responseData = await usersModel.deleteData(req.body, { additionalContext: { requestId: req.id || null } });
    ...
    console.error('Error saat menghapus data users:', error);
    ...
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while deleting users data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// POST /api/guestbook/users/first - Mendapatkan data users berdasarkan kriteria
router.post('/first', async (req, res) => {
  try {
    // req.log.debug('Request body users/first:', JSON.stringify(req.body, null, 2));
    ...
    const validFields = ["id","name","email","password","role","created_at","updated_at"];
    ...
    const result = await usersModel.getData(getPayload);
    ...
    console.error('Error saat mendapatkan data users:', error);
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
...
// GET /api/guestbook/users/lookup - Pencarian users untuk dropdown (dynamic)
router.get('/lookup', async (req, res) => {
  try {
    ...
    console.log(`users lookup request: search=${search}, filters=`, extraFilters);

    // Gunakan model untuk lookup data dengan filters tambahan
    const list = await usersModel.getLookupDataDynamic(search, extraFilters);
    ...
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while looking up users data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// POST /api/guestbook/users/lookup - Mendapatkan data users untuk lookup dengan filtering
router.post('/lookup', async (req, res) => {
  try {
    ...
    if (!req.body) {
      ...
      return res.status(400).json({
        ...
        example: {
          ...
          "where": [{ "key": "name", "value": "AP2506-01" }],
    ...
    // req.log.debug('Request body users/lookup:', JSON.stringify(req.body, null, 2));
    ...
    if (req.body.where) {
      ...
      const list = await usersModel.getLookupDataWithFilter(req.body);
      ...
      console.log(`users static lookup request with selected_tag: ${selectedTag}`);
      ...
      if ((req.body.select && Array.isArray(req.body.select)) || (req.body.sort_columns && Array.isArray(req.body.sort_columns))) {
        ...
        const list = await usersModel.getLookupDataWithFilter(modifiedBody);
        ...
        const list = await usersModel.getStaticLookupData(selectedTag);
    ...
    console.error('Error in users static lookup:', error);
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while fetching users data',
...
// POST /api/guestbook/users/read - Manual pagination endpoint
router.post('/read', async (req, res) => {
  try {
    // req.log.debug('Request body users/read:', JSON.stringify(req.body, null, 2));
    ...
    const result = await usersModel.getList(options);
    ...
    return res.status(statusCode).json({
      success: false,
      error: statusCode === 400 ? 'Bad Request' : 'Internal Server Error',
      message: statusCode === 400 ? error.message : 'An error occurred while fetching users list data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
router.get('/info', async (req, res) => {
  try {
    ...
    const modelInfo = await usersModel.getModelInfo(actions);
    ...
    console.error('Error in users info:', error);
    res.status(500).json({
      success: false,
      error: 'Internal Server Error',
