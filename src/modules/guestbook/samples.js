...
const samplesModel = require('../../models/guestbook/samples');
...
try {
  ...
  if (hasComponents) {
    ...
    const componentConfig = {
      tableName: 'contact',
      fieldName: ["contact_id","contact_name","email","address","is_active","created_at","created_by","updated_at","updated_by"],
      exportQuery: null,
      columnFormats: null,
      fieldLabels: null,
    ...
    componentEngine.loadConfigurationFromObject(componentConfig).then(result => {
      if (result.success) {
        console.log(`Component configuration loaded for guestbook/samples: ${result.componentsLoaded} components`);
      }
    }).catch(err => {
      console.error(`Failed to load component configuration for guestbook/samples:`, err.message);
    });
  } else {
    console.log(`No components defined in payload for guestbook/samples, running without events`);
  }
} catch (e) {
  if (hasComponents) {
    // Components dikonfigurasi tapi engine gagal load — ini error, bukan optional
    console.error(`CRITICAL: Component engine failed to load for guestbook/samples but components are configured:`, e.message);
    throw e;
  }
  // Jika tidak ada components, silent skip wajar
  console.log(`No component engine required for guestbook/samples, running without events`);
}

/**
 * Samples Submodule - Auto-generated on 2026-05-09 08:22:11
 *
 * Endpoints untuk samples dengan actions: datatables, create, update, delete, first, lookup, read, export, import
 * Table: contact
 * Fields: 9 fields
 * Database: PostgreSQL
 */
    ...
    try {
      ...
      if (!req.body || Object.keys(req.body).length === 0) {
        return res.status(400).json({
          ...
          endpoint: '/api/guestbook/samples' + req.path
        });
      }
      ...
      if (endpoint === 'add') {
        ...
        const autoGenerateFields = ["contact_id"];
...
// POST /api/guestbook/samples/datatables - Data untuk DataTables
router.post('/datatables', async (req, res) => {
  try {
    // req.log.debug('Request body samples/datatables:', JSON.stringify(req.body, null, 2));
    ...
    const result = await samplesModel.getDatatables(options);
    ...
    console.error('Error in samples datatables:', error);
    const statusCode = error.statusCode || 500;
    return res.status(statusCode).json({
      success: false,
      error: statusCode === 400 ? 'Bad Request' : 'Internal Server Error',
      message: statusCode === 400 ? error.message : 'An error occurred while fetching samples data',
...
// POST /api/guestbook/samples/create - Menambahkan data samples baru
router.post('/create', async (req, res) => {
  try {
    // req.log.debug('Request body samples/create:', JSON.stringify(req.body, null, 2));
    ...
    if (typeof samplesModel.validateData === 'function') {
      const validation = await samplesModel.validateData(req.body, 'insert');
    ...
    if (componentEngine && ContextBuilder) {
      ...
      try {
        ...
        responseData = await samplesModel.addData(req.body, eventContext);
      ...
      try {
        responseData = await samplesModel.addData(req.body, { additionalContext: { requestId: req.id || null } });
        newData = responseData;
    ...
    console.log(`samples data added successfully: ${responseData.contact_id || 'new record'}`);

    // Kirim response
    return res.status(201).json({
      success: true,
      message: 'samples data successfully added',
      data: responseData,
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error('Error saat menambahkan data samples:', error);
    ...
    return res.status(500).json({
      ...
      message: 'An error occurred while adding samples data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// POST /api/guestbook/samples/update - Mengupdate data samples
router.post('/update', async (req, res) => {
  try {
    // req.log.debug('Request body samples/update:', JSON.stringify(req.body, null, 2));
    ...
    // Validasi data dengan model jika tersedia
    if (typeof samplesModel.validateData === 'function') {
      const validation = await samplesModel.validateData(req.body, 'update');
      if (!validation.isValid) {
        return res.status(400).json({
          success: false,
          error: 'Validation failed',
    ...
    if (componentEngine && ContextBuilder) {
      ...
      try {
        ...
        responseData = await samplesModel.updateData(req.body, eventContext);
        newData = responseData;
      ...
      // Fallback: gunakan mode lama tanpa events (tetap propagasi requestId untuk Live Sync)
      try {
        responseData = await samplesModel.updateData(req.body, { additionalContext: { requestId: req.id || null } });
        newData = responseData;
    ...
    // Log successful operation
    console.log(`samples data updated successfully: ${primaryKey}=${req.body[primaryKey]}`);

    // Kirim response
    return res.status(200).json({
      success: true,
      message: 'samples data successfully updated',
      data: responseData,
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error('Error saat mengupdate data samples:', error);
    ...
    if (error.message === 'Data tidak ditemukan' || error.message.includes('not found')) {
      return res.status(404).json({
        ...
        message: 'samples data not found',
        timestamp: new Date().toISOString()
      });
    }
    ...
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while updating samples data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// POST /api/guestbook/samples/delete - Menghapus data samples
router.post('/delete', async (req, res) => {
  try {
    // req.log.debug('Request body samples/delete:', JSON.stringify(req.body, null, 2));
    ...
    if (req.body.where && Array.isArray(req.body.where) && req.body.where.length > 0) {
      ...
      try {
        const existingData = await samplesModel.getData({
    ...
    if (componentEngine && ContextBuilder) {
      ...
      try {
        ...
        responseData = await samplesModel.deleteData(req.body, eventContext);
      ...
      // Fallback: gunakan mode lama tanpa events (tetap propagasi requestId untuk Live Sync)
      try {
        responseData = await samplesModel.deleteData(req.body, { additionalContext: { requestId: req.id || null } });
    ...
    // Log successful operation
    console.log(`samples data deleted successfully`);
    ...
    console.error('Error saat menghapus data samples:', error);
    ...
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while deleting samples data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// POST /api/guestbook/samples/first - Mendapatkan data samples berdasarkan kriteria
router.post('/first', async (req, res) => {
  try {
    // req.log.debug('Request body samples/first:', JSON.stringify(req.body, null, 2));
    ...
    if (!req.body.where || typeof req.body.where !== 'object' || Array.isArray(req.body.where)) {
      return res.status(400).json({
        ...
        example: {
          "where": { "key": "contact_id", "value": "your-id-value" },
          "select": ["field1", "field2"]
        },
        timestamp: new Date().toISOString()
      });
    ...
    const validFields = ["contact_id","contact_name","email","address","is_active","created_at","created_by","updated_at","updated_by"];
    ...
    const result = await samplesModel.getData(getPayload);
    ...
    console.error('Error saat mendapatkan data samples:', error);
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while fetching samples data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// GET /api/guestbook/samples/lookup - Pencarian samples untuk dropdown (dynamic)
router.get('/lookup', async (req, res) => {
  try {
    ...
    console.log(`samples lookup request: search=${search}, filters=`, extraFilters);

    // Gunakan model untuk lookup data dengan filters tambahan
    const list = await samplesModel.getLookupDataDynamic(search, extraFilters);
    ...
    console.error('Error in samples lookup:', error);
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while looking up samples data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
// POST /api/guestbook/samples/lookup - Mendapatkan data samples untuk lookup dengan filtering
router.post('/lookup', async (req, res) => {
  try {
    ...
    if (!req.body) {
      ...
      const lookupConfig = {"idField":"contact_id","textField":"contact_name||' - '||email as display_text","hasCustomText":true,"searchFields":["contact_name","email"]};
      ...
      return res.status(400).json({
        ...
        example: {
          ...
          "where": [{ "key": "contact_id", "value": "AP2506-01" }],
          "select": defaultSelect
        },
        example_sql_expression: {
          "where": [{ "key": "company_id", "value": "your-company-id" }],
          "select": ["id", "contact_id||' - '||code as display_text"]
        },
        ...
        example_advanced: {
          "where": {
            "logic": "OR",
            "conditions": [
              { "key": "contact_id", "operator": "like", "value": "%Portal%" },
              { "key": "contact_id", "operator": "like", "value": "%system%", "sensitive": false }
            ]
          },
    ...
    // req.log.debug('Request body samples/lookup:', JSON.stringify(req.body, null, 2));
    ...
    if (req.body.where) {
      ...
      if (!Array.isArray(req.body.where) && !req.body.where.conditions) {
        return res.status(400).json({
          ...
          example: {
            "where": [
              { "key": "contact_id", "value": "AP2506-01" }
            ],
            "select": ["id", "contact_id"]
          },
          example_advanced: {
      ...
      const list = await samplesModel.getLookupDataWithFilter(req.body);
      ...
      console.log(`samples static lookup request with selected_tag: ${selectedTag}`);
      ...
      if ((req.body.select && Array.isArray(req.body.select)) || (req.body.sort_columns && Array.isArray(req.body.sort_columns))) {
        ...
        const list = await samplesModel.getLookupDataWithFilter(modifiedBody);
        ...
        const list = await samplesModel.getStaticLookupData(selectedTag);
    ...
    console.error('Error in samples static lookup:', error);
    return res.status(500).json({
      success: false,
      error: 'Internal Server Error',
      message: 'An error occurred while fetching samples data',
...
// POST /api/guestbook/samples/read - Manual pagination endpoint
router.post('/read', async (req, res) => {
  try {
    // req.log.debug('Request body samples/read:', JSON.stringify(req.body, null, 2));
    ...
    const searchBy = req.body.search_by || 'contact_id';
    ...
    const result = await samplesModel.getList(options);
    ...
    console.error('Error in samples list:', error);
    const statusCode = error.statusCode || 500;
    return res.status(statusCode).json({
      success: false,
      error: statusCode === 400 ? 'Bad Request' : 'Internal Server Error',
      message: statusCode === 400 ? error.message : 'An error occurred while fetching samples list data',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
router.get('/info', async (req, res) => {
  try {
    const actions = {"datatables":true,"read":true,"first":true,"create":true,"update":true,"delete":true,"lookup":true,"export":true,"import":true,"workflow":false,"info":true};

    const modelInfo = await samplesModel.getModelInfo(actions);
    ...
    res.json({
      ...
      generated: '2026-05-09 08:22:11',
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error('Error in samples info:', error);
    res.status(500).json({
      success: false,
      error: 'Internal Server Error',
