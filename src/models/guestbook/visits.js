...
/**
 * Visits Model - Auto-generated on 2026-05-09 16:16:55
 *
...
class VisitsModel extends BaseModel {
   ...
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel visits
    const validFields = [
      'id',
      'visit_number',
      'visitor_id',
    ...
    const datatablesWhere = ["visit_number","visitor_code","visitor_name","company","host_name","purpose","status","receptionist_name","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('visits', validFields, datatablesWhere);
    ...
    this.viewName = 'visits';
    this.readSource = 'visits'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'visits'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan visits
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk visits
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'visits-datatables.sql');
  ...
  getReadQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'visits-viewquery.sql');
      if (fs.existsSync(queryFilePath)) {
        baseQuery = fs.readFileSync(queryFilePath, 'utf8').trim();
      } else {
        throw new Error(`Query file not found: ${queryFilePath}`);
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'visit_number' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT id, visit_number FROM ' + this.getLookupSource() + ' WHERE upper(a.visit_number) LIKE upper($1) ORDER BY a.visit_number';
      ...
      const result = data.map(item => ({
        id: item.id,
        text: item.visit_number
      }));

      return result;
    } catch (error) {
      console.error('Error in getLookupData (VisitsModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      console.error('Error in getLookupDataDynamic (VisitsModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'visit_number' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT id, visit_number FROM ' + this.getLookupSource() + ' ORDER BY a.visit_number';
      ...
      console.error('Error in getStaticLookupData (VisitsModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['id', 'visit_number'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["visitor_code","visitor_name","host_name","receptionist_name"];
      let selectClause = 'id';
      let textField = 'visit_number';
      let aliasField = null;
      ...
      for (const column of selectColumns) {
        ...
        if (validTextFields.includes(column) || column === 'visit_number') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      console.log('=== DEBUG VISITS LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.id,
        text: item[textFieldName] || item.visit_number || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (VisitsModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["visitor_code","visitor_name","host_name","receptionist_name"],
      dateFields: ["check_in_time","check_out_time","created_by","created_at","updated_at"],
      requiredFields: ["id","visitor_code","visitor_name","host_name","receptionist_name"],
      primaryTextField: 'visit_number',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk visits
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response visits
   */
...
module.exports = new VisitsModel();