...
/**
 * Samples Model - Auto-generated on 2026-05-09 08:22:11
 *
 * Model untuk samples yang mewarisi fungsi-fungsi dari BaseModel
 * Table: contact
 * Fields: 9 fields
 * Database: PostgreSQL
 */
class SamplesModel extends BaseModel {
  /**
   * Constructor
   */
  constructor() {
    // Definisikan validFields - semua field yang valid untuk tabel samples
    const validFields = [
      'contact_id',
      'contact_name',
      'email',
    ...
    const datatablesWhere = ["contact_name","email","address","all"];

    // Panggil constructor parent dengan nama tabel, validFields, dan datatablesWhere
    super('contact', validFields, datatablesWhere);
    ...
    this.readSource = 'contact'; // Source untuk operasi read (get, list, lookup, datatables)
    this.writeSource = 'contact'; // Source untuk operasi write (add, update, delete)
  ...
  /**
   * Override getListQuery untuk menyesuaikan dengan kebutuhan samples
   * @param {Object} options - Query options
   * @returns {string} SQL query dasar untuk samples
   */
  getListQuery(options = {}) {
    ...
    let baseQuery;
    try {
      const queryFilePath = path.join(__dirname, 'query', 'samples-datatables.sql');
  ...
  /**
   * Override getLookupData untuk menggunakan kolom 'contact_id' yang benar
   * @param {string} search - Kata kunci pencarian
   * @returns {Array} Array objek hasil lookup
   */
  async getLookupData(search) {
    try {
      const query = 'SELECT contact_id, contact_id FROM ' + this.getLookupSource() + ' WHERE upper(a.contact_id) LIKE upper($1) ORDER BY a.contact_id';
      ...
      console.error('Error in getLookupData (SamplesModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataDynamic(search, extraFilters = {}) {
    try {
      ...
      for (const [key, value] of Object.entries(extraFilters)) {
        if (value && ["contact_id","contact_name","email","address","is_active","created_at","created_by","updated_at","updated_by"].includes(key)) {
      ...
      const idField = lookupConfig ? lookupConfig.idField : 'contact_id';
      ...
      return data.map(item => ({
        id: item[idField] || item.contact_id,
        text: item[lookupConfig && lookupConfig.hasCustomText ? 'display_text' : textFields[0]] || ''
      }));
    } catch (error) {
      console.error('Error in getLookupDataDynamic (SamplesModel):', error);
      throw error;
    }
  }

  /**
   * Override getStaticLookupData untuk menggunakan kolom 'contact_id' yang benar
  ...
  async getStaticLookupData(selectedTag) {
    try {
      ...
      const query = 'SELECT contact_id, contact_id FROM ' + this.getLookupSource() + ' ORDER BY a.contact_id';
      ...
      return data.map(item => {
        ...
        if (item.contact_id === selectedTag) {
      ...
      console.error('Error in getStaticLookupData (SamplesModel):', error);
      throw error;
    }
  }
  ...
  async getLookupDataWithFilter(options) {
    try {
      ...
      const selectColumns = options.select || ['contact_id', 'contact_id'];
      let params = [];
      let paramIndex = 1;

      // Parse dan validasi select columns untuk support SQL expressions
      const validTextFields = ["contact_name"];
      let selectClause = 'contact_id';
      let textField = 'contact_id';
      let aliasField = null;

      // Proses setiap column dalam select
      for (const column of selectColumns) {
        if (column.toLowerCase() === 'contact_id'.toLowerCase()) {
        ...
        if (validTextFields.includes(column) || column === 'contact_id') {
          selectClause += `, ${column}`;
          textField = column;
          break; // gunakan yang pertama sebagai text field
        }
      ...
      if (options.sort_columns && Array.isArray(options.sort_columns) && options.sort_columns.length > 0) {
        const orderParts = options.sort_columns.map(item => {
          ...
          if (!this.validFields.includes(column) && column !== 'contact_id') return null;
      ...
      console.log('=== DEBUG SAMPLES LOOKUP WITH FILTER ===');
      ...
      const result = data.map(item => ({
        id: item.contact_id,
        text: item[textFieldName] || item.contact_id || item.name || item.code || item.description || ''
      }));
      ...
      console.error('Error in getLookupDataWithFilter (SamplesModel):', error);
      throw error;
    }
  }
  ...
  getFieldMapping() {
    return {
      ...
      textFields: ["contact_name"],
      dateFields: ["created_at","created_by","updated_at","updated_by"],
      requiredFields: ["contact_name","email"],
      primaryTextField: 'contact_id',
      searchableFields: this.getSearchableColumns ? this.getSearchableColumns().map(col => col.name) : []
    };
  }

  /**
   * Override formatResponseData untuk samples
   * @param {Object} data - Data dari database
   * @returns {Object} Data yang sudah diformat untuk response samples
   */
...
module.exports = new SamplesModel();